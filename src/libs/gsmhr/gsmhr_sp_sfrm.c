...
*/
  ...
  int32_t L_Energy,
         ...
         L_cgNorm,
         L_CrossCorr;
  int32_t pL_CCBuf[MAX_CANDIDATE],
...
void   decorr(int iNumVects, int16_t pswGivenVect[],
                     int16_t pswVects[])
{
...
*/
  ...
  int16_t pswWSVec[S_LEN],
         pswTempVec[S_LEN];
  int16_t pswPVec[S_LEN],
         pswWPVec[S_LEN];
  int16_t ppswVselpEx[2][S_LEN],
         ppswWVselpEx[2][S_LEN];
  int16_t pswWBasisVecs[9 * S_LEN],
         pswBitArray[9];
  ...
  {                                                          /* DTX mode */   
    ...
    for (i = 0; i < siNumBasisVecs; i++)
    {
      ...
      if (swVoicingMode > 0)
      {
        ...
        lpcZsIir((int16_t *) pppsrVcdCodeVec[0][i], pswHCoefs,
                 &pswWBasisVecs[i * S_LEN]);
      }
      else
      {

        lpcZsIir((int16_t *) pppsrUvCodeVec[0][i], pswHCoefs,
                 &pswWBasisVecs[i * S_LEN]);
      }
    ...
    *psiVSCode1 = v_srch(pswWSVec, pswWBasisVecs, siNumBasisVecs);
    ...
    if (swVoicingMode > 0)
      v_con((int16_t *) pppsrVcdCodeVec[0][0], ppswVselpEx[0], pswBitArray,
            siNumBasisVecs);

    else
      v_con((int16_t *) pppsrUvCodeVec[0][0], ppswVselpEx[0], pswBitArray,
            siNumBasisVecs);
    ...
    if (swVoicingMode == 0)
    {
      ...
      for (i = 0; i < siNumBasisVecs; i++)
      {
        ...
        lpcZsIir((int16_t *) pppsrUvCodeVec[1][i], pswHCoefs,
                 &pswWBasisVecs[i * S_LEN]);
      }
      ...
      *psiVSCode2 = v_srch(pswWSVec, pswWBasisVecs, siNumBasisVecs);
      ...
      v_con((int16_t *) pppsrUvCodeVec[1][0], ppswVselpEx[1], pswBitArray,
            siNumBasisVecs);
    }
