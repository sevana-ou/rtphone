                      ...
                      int optmask, socket_function_ptr socketFunc)
{
 ...
 //  struct timeval tv;
  ...
  if (status == ARES_SUCCESS || !channel->nservers)
  {
  ...
  if (channel->nservers == -1)
    {
      ...
      */
      ...
      typedef DWORD (WINAPI *GetNetworkParamsType)(FIXED_INFO*, DWORD*);
      GetNetworkParamsType GetNetworkParams = NULL;
      ...
      GetNetworkParams = (GetNetworkParamsType)GetProcAddress(hLib, "GetNetworkParams");
	  if(!GetNetworkParams)
	  {
		  FreeLibrary(hLib);
		  return ARES_ENOTIMP;
...
{
#ifdef WIN32
  typedef DWORD (WINAPI * GetAdaptersAddressesType)(ULONG, DWORD, VOID *, IP_ADAPTER_ADDRESSES *, ULONG *);
  GetAdaptersAddressesType GetAdaptersAddressesProc = NULL;
  ...
  *physAddrLen = 0;
  ...
  GetAdaptersAddressesProc = (GetAdaptersAddressesType)GetProcAddress(hLib, "GetAdaptersAddresses");
