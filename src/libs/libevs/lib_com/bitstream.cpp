...
)
{
    ...
    else if ( total_brate != FRAME__NO_DATA )
    {
        st->Opt_AMR_WB = 0;
    }
...
)
{
    ...
    if (st->total_brate == FRAME__NO_DATA)
    {
...
)
{
    ...
    if ( st->total_brate == FRAME__NO_DATA || st->total_brate == SID_2k40 )
    {
        st->core = ACELP_CORE;
    }
    /* SC-VBR */
...
short read_indices(                   /* o  : 1 = reading OK, 0 = problem            */
    Decoder_State *st,                /* i/o: decoder state structure                */
    const char* data,                /* i  : bitstream file                         */
    size_t data_size,
...
#define INCREASE_DATA_OFFSET(X) (*data_processed = *data_processed + X)
    ...
    do
    {
        ...
        if (*data_processed + 2 > data_size)
            return 0;
        utmp = *(const unsigned short*)data; INCREASE_DATA_OFFSET(2);
        ...
        if (*data_processed + 2 > data_size)
            return 0;
        num_bits = *(const unsigned short*)(data + *data_processed); INCREASE_DATA_OFFSET(2);
        ...
        if (*data_processed + num_bits * sizeof(short) > data_size)
            return 0;
        memmove(pt_stream, data + *data_processed, num_bits * sizeof(short)); INCREASE_DATA_OFFSET(num_bits * sizeof(short));
    ...
    if( !rew_flag )
    {
        /* handle SID_FIRST, SID_BAD, SPEECH_LOST,  _NO_DATA as properly  as possible for the ITU-T  G.192 format  */
        ...
        {
            sid_upd_bad     = 0;
            total_brate     = FRAME__NO_DATA ;         /* treat SID_BAD  as a  stolen signaling frame --> SPEECH LOST */

        }

        g192_sid_first = 0;
        if( st->core == AMR_WB_CORE && st->prev_ft_speech && total_brate == FRAME__NO_DATA && st->bfi == 0 )
        {
            ...
            and  a  good length 0  "SID_FIRST"(_NO_DATA)   frame is sent in AMRWBIO,
        ...
        speech_lost = 0;
        if( total_brate == 0 && st->bfi != 0 ) /*  unsent  _NO_DATA or stolen _NO_DATA/signaling  frame  */
        {
        ...
        /*  handle the  G.192   _simulated_ untransmitted _NO_DATA frame,  setting  for decoder  SPEECH synthesis  */
...
Word16 read_indices_mime(                /* o  : 1 = reading OK, 0 = problem            */
    Decoder_State *st,                   /* i/o: decoder state structure                */
    const char* data,                    /* i  : bitstream data                         */
    size_t data_size,                    /* i  : bitstream data size                    */
    size_t* data_processed,              /* o  : processed data size                    */
...
)
{
    ...
    /* read the FT header */
    if (*data_processed + 1 > data_size)
        return 0;
    header = *(UWord8*)data; INCREASE_DATA_OFFSET(1);
    ...
    if ( core_mode  == 15)
    {
        /*  _NO_DATA unsent CNG frame OR  any frame marked or injected  as _NO_DATA  by e.g a signaling layer or dejitter buffer */
        _NO_DATA=1;
    }
    ...
    memmove(pFrame, data + *data_processed, num_bytes_read); INCREASE_DATA_OFFSET(num_bytes_read);
    ...
    if ( rew_flag )
    {
        ...
        if( st->bfi == 0 && speech_lost == 0 && _NO_DATA == 0 )
        {
    ...
    if( !rew_flag )
    {
        ...
        /*   in SPEECH synthesis, make sure to activate speech plc for a received _NO_DATA frame,
        ...
        if ( (st->CNG == 0)  &&  ( _NO_DATA != 0 )  )
        {
            /*  treat _NO_DATA received in speech synthesis as  SP_LOST frames, SPEECH PLC code will now become active */
            st->bfi = 1;
            /* total_brate= 0;    always zero for _NO_DATA */
        }

        /* in CNG  */
        /* handle bad speech frame(and bad sid frame) in the decoders CNG synthesis settings pair (total_brate, bfi)  */
        if( ( st->CNG != 0 && ( speech_bad || speech_lost || _NO_DATA ))  || /* SP_BAD or SPEECH_LOST)   --> stay in CNG */
...
)
{
    ...
    st->BER_detect = 0;
    bit0 = 0;
    /* There is no FRAME__NO_DATA or BAD frame indicator in RTP, frames are just missing.
     * In case of comfort noise handle missing frame as FRAME__NO_DATA, otherwise use PLC. */
    if(num_bits != 0)
    {
        st->bfi = 0;
