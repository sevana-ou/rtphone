...
)
{
    ...
    /*------------------------------------------------------------------------*
     * Select SID or FRAME__NO_DATA frame if DTX is enabled
     *------------------------------------------------------------------------*/
            ...
            st->fd_cng_reset_flag == 0 )
    {
        ...
        else
        {
            ...
            st->core_brate = FRAME__NO_DATA;
        }

        if( st->core_brate == FRAME__NO_DATA && st->last_core != ACELP_CORE && !st->Opt_AMR_WB )
        {
    ...
    /*------------------------------------------------------------------------*
     * Reset counters when in active frame (neither SID nor FRAME__NO_DATA frame)
     *------------------------------------------------------------------------*/
    ...
    if(  st->Opt_DTX_ON && (st->core_brate == SID_2k40 || st->core_brate == FRAME__NO_DATA ) )
    {
        st->bwidth = st->last_bwidth;
...
)
{
    ...
    if( st->core_brate == SID_2k40 || st->core_brate == SID_1k75 || st->core_brate == FRAME__NO_DATA )
    {
