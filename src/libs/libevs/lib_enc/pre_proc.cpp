...
)
{
    ...
    *coder_type = find_uv( st, pitch_fr, voicing_fr, voicing, inp_12k8, *localVAD, ee,
    ...
    else if( st->rf_mode && st->core_brate != FRAME__NO_DATA && st->core_brate != SID_2k40 )
    {
    ...
    else
    {
        st->rf_mode = 0;
        st->codec_mode = MODE1;
        st->rf_indx_frametype[0] = RF__NO_DATA;
    ...
    {
        if( st->core_brate == FRAME__NO_DATA )
        {
    ...
    if( st->codec_mode == MODE1 )
    {
        ...
        if( st->last_L_frame != st->L_frame && st->core_brate != FRAME__NO_DATA && st->core_brate != SID_2k40 && (st->coder_type_raw != VOICED) )
        {
    ...
    *sharpFlag = 0;
    ...
    if( ( ((st->tcxonly == 0) || !(st->core_brate != FRAME__NO_DATA || st->core_brate != SID_2k40)) && st->L_frame == L_FRAME16k && st->codec_mode == MODE2 ) ||
    ...
    if( st->codec_mode == MODE2 )
    {
        if( st->core_brate != FRAME__NO_DATA && st->core_brate != SID_2k40 && st->tcxonly == 0 )
        {
