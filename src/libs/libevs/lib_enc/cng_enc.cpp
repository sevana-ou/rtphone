...
)
{
    ...
    *allow_cn_step = ((st->cng_cnt == 0) &&
    ...
    if( st->last_core_brate == FRAME__NO_DATA || st->last_core_brate == SID_1k75 || st->last_core_brate == SID_2k40 )
    {
...
)
{
    ...
    if( st->core_brate == SID_2k40 || st->core_brate == FRAME__NO_DATA )
    {
...
)
{
    ...
    if ( st->core_brate == SID_2k40 )
    {
        ...
        else if ( st->shb__NO_DATA_cnt >= 100 )
        {
            update = 1;
        }
