...
)
{
    ...
    {
        /*----------------------------------------------------------------*
         * Processing of FRAME__NO_DATA frames
         * Decoding of SID frames
         *----------------------------------------------------------------*/

        if ( st->core_brate == FRAME__NO_DATA || st->core_brate == SID_1k75 )
        {
        ...
        else
        {
            ...
            FEC_clas_estim( syn, pitch_buf, st->L_frame, coder_type, st->codec_mode, st->mem_syn_clas_estim, &st->clas_dec,
                            ...
                            &st->LT_UV_cnt, &st->Last_ener, &amr_io_class, st->lt_diff_etot, &class_para, 0, 0, (SIGNAL_CLASSIFIER_MODE)0,
